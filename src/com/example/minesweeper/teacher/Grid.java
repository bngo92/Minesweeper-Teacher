...
public class Grid {
	...
	public void randomize() {
		...
		ArrayList<Pair<Integer, Integer>> grid = new ArrayList<Pair<Integer, Integer>>(
				height * width);
		for (int r = 0; r < height; r++) {
			for (int c = 0; c < width; c++) {
		...
		for (int r = 0; r < height; r++) {
			for (int c = 0; c < width; c++) {
				...
				for (int rr = Math.max(r - 1, 0); rr < Math.min(r + 2, height); rr++) {
					for (int cc = Math.max(c - 1, 0); cc < Math.min(c + 2,
							width); cc++) {
	 ...
	 * @param c
	 *            column of tile
	 * @return -1 if clicked on a mine, count of unrevealed tiles on the grid
	 *         otherwise
	 */
	...
	/**
	 * Attempts to reveal tile. Activates all mines if a mine is clicked.
	 * Attempts to reveal surroundings if possible.
	 * 
	 * @param r
	 *            row of tile to reveal
	 * @param c
	 *            column of tile to reveal
